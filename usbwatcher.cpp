...
USBwatcher::USBwatcher(QObject *parent):QThread(parent)
{
    ...
    connect(watcher, &QDeviceWatcher::deviceAdded, this, [](QString s){qDebug() << "\n\n\nadded" << s;}, Qt::DirectConnection);
    connect(watcher, &QDeviceWatcher::deviceChanged, this, [](QString s){qDebug() << "\n\n\nchanged" << s;}, Qt::DirectConnection);
    connect(watcher, &QDeviceWatcher::deviceRemoved, this, [](QString s){qDebug() << "\n\n\nremoved" << s;}, Qt::DirectConnection);
    watcher->start();
}
